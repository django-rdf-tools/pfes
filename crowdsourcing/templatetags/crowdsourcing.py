...
def show_comments(comments_list, question):
    out = '<ul class="scomments">'
    for c in comments_list[question]:
        out = out + '<li><span>'+ c[0]+' : </span>'+ linebreaksbr(c[1]) +'</li>'
    out = out + '</ul>'
    return out
register.simple_tag(show_comments)
